...
const DATA_TYPES = {
  TMAX: 'TMAX', // Maximum temperature (already in Fahrenheit)
  TMIN: 'TMIN', // Minimum temperature (already in Fahrenheit)
  PRCP: 'PRCP', // Precipitation (already in inches)
  SNOW: 'SNOW', // Snowfall (already in inches)
  AWND: 'AWND'  // Average daily wind speed (already in mph)
};
...
async function fetchYearData(county, year) {
    ...
    try {
      ...
      for (let month = 1; month <= 12; month++) {
        ...
        const result = await makeRequest('data', {
          ...
          limit: 1000,
          units: 'standard' // Use standard units (imperial)
        });
  ...
  const processedData = processRawData(allStationData, year, county.name);
...
function processRawData(rawData, year, countyName) {
  // First, log some sample records to understand the data format
  console.log("\nSample raw records for debugging:");
  if (rawData.length > 0) {
    const sampleRecords = rawData.slice(0, 3);
    console.log(JSON.stringify(sampleRecords, null, 2));
  }
  
  // Organize data by date and data type
  const dataByDate = {};
  ...
  dates.forEach(date => {
    ...
    // CORRECTED UNIT CONVERSIONS - Values already appear to be in imperial units
    const record = {
      startTime: `${date}T00:00:00Z`,
      values: {
        // Temperature: NOAA appears to be providing values already in Fahrenheit
        // No conversion needed
        temperatureMax: dayData.TMAX !== undefined ? dayData.TMAX : null,
        temperatureMin: dayData.TMIN !== undefined ? dayData.TMIN : null,
        
        // Precipitation: NOAA appears to be providing values in inches
        // No conversion needed
        precipitationIntensity: dayData.PRCP !== undefined ? dayData.PRCP : 0,
        
        // Wind speed: NOAA appears to be providing values in mph
        // No conversion needed
        windSpeed: dayData.AWND !== undefined ? dayData.AWND : null,
        
        // Snowfall: NOAA appears to be providing values in inches
        // No conversion needed
        snowfall: dayData.SNOW !== undefined ? dayData.SNOW : 0
      }
    };
    
    // Log a sample of the converted data for verification
    if (date === dates[0] || date === dates[Math.floor(dates.length / 2)] || date === dates[dates.length - 1]) {
      console.log(`\nSample converted data for ${date}:`);
      console.log(`  Raw TMAX: ${dayData.TMAX}, Converted: ${record.values.temperatureMax}°F`);
      console.log(`  Raw TMIN: ${dayData.TMIN}, Converted: ${record.values.temperatureMin}°F`);
      console.log(`  Raw PRCP: ${dayData.PRCP}, Converted: ${record.values.precipitationIntensity} inches`);
      console.log(`  Raw AWND: ${dayData.AWND}, Converted: ${record.values.windSpeed} mph`);
      console.log(`  Raw SNOW: ${dayData.SNOW}, Converted: ${record.values.snowfall} inches`);
    }
  ...
  // Get stations for this county
  const stations = countyStations[countyName] || [];
...
function processDailyMetrics(dailyData, year, countyData) {
  ...
  dailyData.forEach(day => {
    ...
    // --- Drought Analysis ---
    // CORRECTED: Using appropriate threshold for drought (0.01 inches is trace rainfall)
    if (dailyRainfall < 0.01) { 
...
function generateInsuranceThresholds() {
  ...
  // Calculate regional climate norms to use as fallbacks
  const regionalNorms = {
    drought: { streaks: [] },
    heatWave: { streaks: [] },
    excessiveRain: { maxValues: [] },
    wind: { maxValues: [] }
  };
    ...
    try {
      ...
      for (let year = currentYear - 10; year < currentYear; year++) {
        if (metrics.droughtMetrics[year]) {
          ...
          // Add to regional norms
          regionalNorms.drought.streaks.push(...metrics.droughtMetrics[year].droughtStreaks);
        }
        
        if (metrics.heatStressMetrics[year]) {
          heatStreaks.push(...metrics.heatStressMetrics[year].heatStreaks);
          // Add to regional norms
          regionalNorms.heatWave.streaks.push(...metrics.heatStressMetrics[year].heatStreaks);
        }
        
        if (metrics.excessRainMetrics[year]) {
          maxRainfalls.push(metrics.excessRainMetrics[year].maxDailyRainfall);
          // Add to regional norms
          regionalNorms.excessiveRain.maxValues.push(metrics.excessRainMetrics[year].maxDailyRainfall);
        }
        
        if (metrics.windEventMetrics[year]) {
          windSpeeds.push(metrics.windEventMetrics[year].maxWindSpeed);
          // Add to regional norms
          regionalNorms.wind.maxValues.push(metrics.windEventMetrics[year].maxWindSpeed);
        }
      }
      ...
      // Calculate thresholds - using 75th, 90th and 95th percentiles with improved function
      thresholds.counties[county.name] = {
        drought: calculateImprovedPercentileThresholds(droughtStreaks, 'drought'),
        heatWave: calculateImprovedPercentileThresholds(heatStreaks, 'heatWave'),
        excessiveRain: calculateImprovedPercentileThresholds(maxRainfalls, 'excessiveRain'),
        wind: calculateImprovedPercentileThresholds(windSpeeds, 'wind')
      };
  ...
  // Calculate regional norms for fallback values
  regionalNorms.drought.streaks.sort((a, b) => a - b);
  regionalNorms.heatWave.streaks.sort((a, b) => a - b);
  regionalNorms.excessiveRain.maxValues.sort((a, b) => a - b);
  regionalNorms.wind.maxValues.sort((a, b) => a - b);
  
  const regionalThresholds = {
    drought: calculateImprovedPercentileThresholds(regionalNorms.drought.streaks, 'drought'),
    heatWave: calculateImprovedPercentileThresholds(regionalNorms.heatWave.streaks, 'heatWave'),
    excessiveRain: calculateImprovedPercentileThresholds(regionalNorms.excessiveRain.maxValues, 'excessiveRain'),
    wind: calculateImprovedPercentileThresholds(regionalNorms.wind.maxValues, 'wind')
  };
  
  // Calculate average thresholds for normalization
  const avgWindModerate = Object.values(thresholds.counties)
    .map(c => c.wind.moderate)
    .filter(v => v > 0)
    .reduce((sum, v) => sum + v, 0) / Object.values(thresholds.counties)
      .map(c => c.wind.moderate)
      .filter(v => v > 0).length;
      
  const avgRainModerate = Object.values(thresholds.counties)
    .map(c => c.excessiveRain.moderate)
    .filter(v => v > 0)
    .reduce((sum, v) => sum + v, 0) / Object.values(thresholds.counties)
      .map(c => c.excessiveRain.moderate)
      .filter(v => v > 0).length;
  
  // Normalize and fix threshold issues
  Object.keys(thresholds.counties).forEach(countyName => {
    // Fix heat wave thresholds when missing data
    if (thresholds.counties[countyName].heatWave.moderate === 0) {
      console.log(`Fixing missing heat wave data for ${countyName} County...`);
      thresholds.counties[countyName].heatWave = regionalThresholds.heatWave;
      
      // If regional data is also missing, use minimum reasonable values
      if (thresholds.counties[countyName].heatWave.moderate === 0) {
        thresholds.counties[countyName].heatWave = {
          moderate: 3, // 3 consecutive days is minimum for heat wave
          severe: 5,   // 5 days is severe
          extreme: 7    // 7 days is extreme
        };
      }
    }
    
    // Check for extreme outliers in wind data - normalize if needed
    if (Math.abs(thresholds.counties[countyName].wind.moderate - avgWindModerate) > avgWindModerate * 0.5) {
      console.log(`Normalizing outlier wind data for ${countyName} County...`);
      thresholds.counties[countyName].wind = {
        moderate: parseFloat(avgWindModerate.toFixed(1)),
        severe: parseFloat((avgWindModerate * 1.2).toFixed(1)),
        extreme: parseFloat((avgWindModerate * 1.4).toFixed(1))
      };
    }
    
    // If rain thresholds are outliers (>50% from average), normalize them
    if (Math.abs(thresholds.counties[countyName].excessiveRain.moderate - avgRainModerate) > avgRainModerate * 0.5) {
      console.log(`Normalizing outlier rain data for ${countyName} County...`);
      thresholds.counties[countyName].excessiveRain = {
        moderate: parseFloat(avgRainModerate.toFixed(1)),
        severe: parseFloat((avgRainModerate * 1.3).toFixed(1)),
        extreme: parseFloat((avgRainModerate * 1.5).toFixed(1))
      };
    }
  });
...
// Improved helper function to calculate percentile thresholds with proper separation
function calculateImprovedPercentileThresholds(values, metricType) {
  if (values.length === 0) {
  ...
  // Get base values
  const moderate = parseFloat((values[p75Index] || 0).toFixed(1));
  let severe = parseFloat((values[p90Index] || 0).toFixed(1));
  let extreme = parseFloat((values[p95Index] || 0).toFixed(1));
  
  // Ensure minimum differences between thresholds based on metric type
  const minDiff = getMinimumDifferenceForMetric(metricType);
  
  // Ensure increasing thresholds with minimum differences
  severe = Math.max(severe, moderate + minDiff);
  extreme = Math.max(extreme, severe + minDiff);
  
  return { moderate, severe, extreme };
}

// Helper function to get appropriate minimum differences for each metric type
function getMinimumDifferenceForMetric(metricType) {
  switch(metricType) {
    case 'drought':
      return 1; // At least 1 day difference for drought streaks
    case 'heatWave':
      return 1; // At least 1 day difference for heat wave streaks
    case 'excessiveRain':
      return 0.2; // At least 0.2 inch difference for rain thresholds
    case 'wind':
      return 1.0; // At least 1 mph difference for wind thresholds
    default:
      return 0.5; // Default minimum difference
  }
}
